...
export default function WishesForm({ onAddWish }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim() || !wish.trim()) {
      setStatus('error-validation'); // Menambahkan status untuk validasi
      return;
    }
    setSubmitting(true);
    setStatus(null); // Reset status sebelum pengiriman baru

    try {
      await onAddWish({ name: name.trim(), text: wish.trim(), time: new Date().toISOString() });
      setName('');
      setWish('');
      setStatus('success');
    } catch (err) {
      console.error('Wishes submission error:', err); // Log error
      setStatus('error');
    } finally {
      setSubmitting(false);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <motion.input
            ...
            type="text"
            id="wisher-name" // ID unik untuk label
            name="name"
            value={name}
        ...
        <div>
          ...
          <motion.textarea
            whileFocus={{ scale: 1.01, boxShadow: '0 0 0 3px rgba(113, 113, 122, 0.5)' }} // Glow abu-abu
            transition={{ type: 'spring', stiffness: 300 }}
            id="wisher-wish" // ID unik untuk label
            name="wish"
            value={wish}
            ...
            className="w-full p-3 rounded-md bg-zinc-800 text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-600 resize-y transition-all duration-200 border border-zinc-700" // Input lebih gelap, izinkan resize vertikal
          />
        </div>
